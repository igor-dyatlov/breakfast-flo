/*
 * WCD9310 Sound Volume Controller
 *
 ...
 * See tabla_write function for more info.
 */
...
int tabla_write(struct snd_soc_codec *codec,
	...
	switch (reg) {
		...
		case TABLA_A_CDC_TX5_VOL_CTL_GAIN:
			...
			if (snd_ctrl_locked)
				ret = 0;
			break;
		/* Incall microphone's register */
		case TABLA_A_CDC_TX6_VOL_CTL_GAIN:
			/* Prevent access if it is locked */
			if (snd_ctrl_locked)
				ret = 0;
			break;
		/* Camera microphone's registers */
		case TABLA_A_CDC_TX7_VOL_CTL_GAIN:
			/* Prevent access if it is locked */
			if (snd_ctrl_locked)
				ret = 0;
				     ...
				     const char *buf, size_t count)
{
	...
	if (selected_reg != 0xdeadbeef)
		tabla_write(snd_engine_codec_ptr, selected_reg, val);
...
MODULE_DESCRIPTION("WCD9310 Sound Volume Controller");
