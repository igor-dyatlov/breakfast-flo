/*
 * Copyright (C) 2012-2017, Alucard_24@XDA
 *
...
static void __ref hotplug_work_fn(struct work_struct *work)
{
	...
	unsigned int min_freq = CPUFREQ_MAX_FREQ;
	...
	for_each_online_cpu(cpu) {
		...
		/*
		 * If wall_time < idle_time or wall_time == 0, evaluate cpu load
		 * next time
		 */
	...
	avg_freq = adjust_avg_freq(0, (sum_freq / n));
	if (avg_freq) {
		avg_load = (sum_load / n);
		...
		if (downcpu > 0	&& online_cpus > upmaxcoreslimit) {
			cpu_down(downcpu);
		} else if (online_cpus < min_cpus_online &&
			   offcpu < upmaxcoreslimit) {
			cpu_up(offcpu);
		} else if (offcpu > 0  && offcpu < upmaxcoreslimit &&
			   online_cpus < upmaxcoreslimit &&
			   avg_load >= pcpu_parm->up_load &&
			   avg_freq >= pcpu_parm->up_freq &&
			   (rq_avg > pcpu_parm->up_rq || rq_avg_calc == false)) {
			if (pcpu_parm->cur_up_rate % pcpu_parm->up_rate == 0) {
				cpu_up(offcpu);
			} else {
			  ...
			  (rq_avg <= pcpu_parm->down_rq ||
			   rq_avg_calc == false)))) {
			if (pcpu_parm->cur_down_rate % pcpu_parm->down_rate == 0) {
				cpu_down(downcpu);
			} else {
...
static int hotplug_start(void)
{
	...
	for_each_possible_cpu(cpu) {
		struct hotplug_cpuinfo *pcpu_info = NULL;
		struct hotplug_cpuparm *pcpu_parm = &per_cpu(ac_hp_cpuparm, cpu);
		unsigned int prev_load;
...
{									\
	...
	pcpu_parm = &per_cpu(ac_hp_cpuparm, num_core - 1);		\
									\
	if (val == pcpu_parm->var_name)					\
		return count;						\
									\
	pcpu_parm->var_name = val;					\
									\
	return count;							\
}
				    ...
				    const char *buf, size_t count)
{
	...
	if (val == hotplug_tuners_ins.hotplug_enable)
		return count;

	hotplug_tuners_ins.hotplug_enable = val;
...
static ssize_t store_hotplug_sampling_rate(struct kobject *kobj,
					   struct attribute *attr,
					   const char *buf, size_t count)
{
	int ret;
	...
	if (val == hotplug_tuners_ins.hotplug_sampling_rate)
		return count;

	hotplug_tuners_ins.hotplug_sampling_rate = val;
				     ...
				     const char *buf, size_t count)
{
	...
	if (val == hotplug_tuners_ins.min_cpus_online)
		return count;

	hotplug_tuners_ins.min_cpus_online = val;

	return count;
}

/* maxcoreslimit */
static ssize_t store_maxcoreslimit(struct kobject *kobj,
				   struct attribute *attr,
				   const char *buf, size_t count)
{
	int ret;
	...
	if (val == hotplug_tuners_ins.maxcoreslimit)
		return count;

	if (hotplug_tuners_ins.min_cpus_online > val)
		hotplug_tuners_ins.min_cpus_online = val;

	hotplug_tuners_ins.maxcoreslimit = val;

	return count;
}

/* maxcoreslimit_sleep */
static ssize_t store_maxcoreslimit_sleep(struct kobject *kobj,
					 struct attribute *attr,
					 const char *buf, size_t count)
{
	int ret;
	...
	if (val == hotplug_tuners_ins.maxcoreslimit_sleep )
		return count;

	hotplug_tuners_ins.maxcoreslimit_sleep = val;
 ...
 */
static ssize_t store_hotplug_suspend(struct kobject *kobj,
				     struct attribute *attr,
				     const char *buf, size_t count)
{
	int ret;
	...
	if (val == hotplug_tuners_ins.hotplug_suspend)
		return count;

	hotplug_tuners_ins.hotplug_suspend = val;
...
static int __init alucard_hotplug_init(void)
{
	...
	if (ret) {
		printk(KERN_ERR "Failed to create sysfs");
		return ret;
	}
